...
func inconsistentStatus(set *appsv1beta1.StatefulSet, status *appsv1beta1.StatefulSetStatus) bool {
	...
	for i, v := range status.VolumeClaimTemplates {
		volumeClaimName2StatusIdx[v.VolumeClaimName] = i
	}
	for _, v := range set.Status.VolumeClaimTemplates {
		if idx, ok := volumeClaimName2StatusIdx[v.VolumeClaimName]; !ok {
			// raw template not exist in current status => inconsistent
			return true
		} else if status.VolumeClaimTemplates[idx].CompatibleReplicas != v.CompatibleReplicas ||
			status.VolumeClaimTemplates[idx].CompatibleReadyReplicas != v.CompatibleReadyReplicas {
			return true
		}
	}
	return false
