...
func defaultCheckContainersInPlaceUpdateCompleted(pod *v1.Pod, inPlaceUpdateState *appspub.InPlaceUpdateState) error {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.InPlaceWorkloadVerticalScaling) {
		ljson, _ := json.Marshal(inPlaceUpdateState.LastContainerStatuses)
		sjson, _ := json.Marshal(pod)
		klog.V(5).InfoS("[pre]Check resources in-place update completed",
			"namespace", pod.Namespace, "name", pod.Name,
			"last", ljson, "status", sjson)
		...
		ljson, _ = json.Marshal(inPlaceUpdateState.LastContainerStatuses)
		sjson, _ = json.Marshal(pod)
		klog.V(5).InfoS("[after]Check resources in-place update completed",
			"namespace", pod.Namespace, "name", pod.Name,
			"last", ljson, "status", sjson)
	}
	...
	if runtimeContainerMetaSet != nil {
		...
		if utilfeature.DefaultFeatureGate.Enabled(features.InPlaceWorkloadVerticalScaling) {
			// if vertical scaling is enabled, we should compare plainHashWithoutResources
